...
int tcp_connect_server(struct connection *conn, int data, int delack)
{
	...
	if (fd == -1) {
		...
		if (errno == ENFILE) {
			...
			LOG_HCHK_CONNECTION(conn, fd, "connect error %s", conn_err_code_str(conn));
	...
	if (connect(fd, (struct sockaddr *)&conn->addr.to, get_addr_len(&conn->addr.to)) == -1) {
		LOG_CONNECTION(conn, fd, "connect errno = %d", errno); /* XXX this is going to be noisy */
...
int tcp_connect_probe(struct connection *conn)
{
	...
	if (connect(fd, (struct sockaddr *)&conn->addr.to, get_addr_len(&conn->addr.to)) < 0) {
		LOG_HCHK_CONNECTION(conn, fd, "connect error %d", errno);
	 ...
	 * data layer.
	 */
	LOG_HCHK_CONNECTION(conn, fd, "connected");
	conn->flags &= ~CO_FL_WAIT_L4_CONN;
	return 1;
 ...
 out_error:
	...
	LOG_HCHK_CONNECTION(conn, fd, "connection write error");
